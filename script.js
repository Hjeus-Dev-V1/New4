...
function renderPlaylist() {
    ...
    tracks.forEach((track, index) => {
        ...
        `;
        item.querySelector(".track-no")?.remove();
    ...
    try {
        ...
        const attempt = () => {
            ...
            if (playPromise && typeof playPromise.catch === "function") {
                playPromise.catch(() => {
                    // Retry on next user interaction or load event.
                });
            }
        };
        bgVideo.addEventListener("loadedmetadata", attempt, { once: true });
        bgVideo.addEventListener("loadeddata", attempt, { once: true });
        bgVideo.addEventListener("canplay", attempt, { once: true });
        bgVideo.addEventListener("canplaythrough", attempt, { once: true });
...
/* ===============================
   AVATAR FRAME VIDEO FORCE-PLAY
============================== */

const avatarFrameVideo = document.querySelector(".avatar-frame-video");

function ensureAvatarFrameVideo() {
    if (!avatarFrameVideo) return;
    try {
        avatarFrameVideo.muted = true;
        avatarFrameVideo.defaultMuted = true;
        avatarFrameVideo.setAttribute("muted", "");
        avatarFrameVideo.playsInline = true;
        avatarFrameVideo.setAttribute("playsinline", "");
        avatarFrameVideo.setAttribute("webkit-playsinline", "");
        avatarFrameVideo.autoplay = true;
        avatarFrameVideo.loop = true;
        avatarFrameVideo.preload = "auto";
        avatarFrameVideo.controls = false;
        avatarFrameVideo.disablePictureInPicture = true;
        avatarFrameVideo.style.visibility = "visible";
        avatarFrameVideo.style.background = "transparent";
        const attempt = () => {
            const playPromise = avatarFrameVideo.play();
            if (playPromise && typeof playPromise.catch === "function") {
                playPromise.catch(() => {});
            }
        };
        avatarFrameVideo.addEventListener("loadedmetadata", attempt, { once: true });
        avatarFrameVideo.addEventListener("loadeddata", attempt, { once: true });
        avatarFrameVideo.addEventListener("canplay", attempt, { once: true });
        avatarFrameVideo.addEventListener("canplaythrough", attempt, { once: true });
        window.addEventListener("load", attempt, { once: true });
        document.addEventListener("visibilitychange", () => {
            if (!document.hidden) attempt();
        });
        ...
        // Ignore and keep the fallback avatar image.
    }
}
ensureAvatarFrameVideo();
